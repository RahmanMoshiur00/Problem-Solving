...
void solve(int tc)
{
    ...
    int v[m];
    for(int i=0; i<m; i++) sfi(v[i]);

    int del = 0, i, j, cnt = 0, i_block, j_block, ans = 0;
    for(i=0; i<m; i++){
        cnt = 1;
        i_block = find_block(v[i]-del, k);
        for(j=i+1; j<m; j++){
            j_block = find_block(v[j]-del, k);
            if(i_block == j_block) cnt++;
            else break;
        }
        i = j-1;
        del += cnt;
