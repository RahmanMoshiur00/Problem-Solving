...
void solve(int tc)
{
    ...
    multiset < char > pst;
    for(int i=0, sz = p.size(); i<sz; i++) pst.insert(p[i]);

    int sat = 0, tat = 0, pat = 0;
    for(int i=0, sz = t.size(), ssz = s.size(); i<sz; i++){
        if(sat<ssz && t[i]==s[sat]){
            sat++;
        }
        else{
            auto it = pst.find(t[i]);
            if(it == pst.end()){
                cout << "NO" << endl;
                return;
            }
