...
vector< pair<int, int> > mat[mxn][mxn];
int a[mxn][mxn], b[mxn][mxn], n, m;
...
void solve(int tc)
{
    ...
    rep(i, 0, n) rep(j, 0, m) sfi(a[i][j]), mat[i][j].push_back({a[i][j], 1});

    rep(i, 0, n) rep(j, 0, m) sfi(b[i][j]), mat[i][j].push_back({b[i][j], 2});

    rep(i, 0, n) rep(j, 0, m) sort(all(mat[i][j]));

    rep(i, 0, n){
        rep(j, 0, m){
            a[i][j] = mat[i][j][0].first;
            b[i][j] = mat[i][j][1].first;
        }
    }
