...
void solve(int tc)
{
    ...
    rep(i, 1, n) bcum[i] = bcum[i-1] + b[i];
    rep(i, 1, m) gcum[i] = gcum[i-1] + g[i];

    int mxb = *max_element(all(b));
    int mng = *min_element(all(g));
    ...
    int ans = 0LL, up, low;
    per(i, n, 0){
        up = upper_bound(all(g), b[i]) - g.begin();
        low = lower_bound(all(g), b[i]) - g.begin();
        ans += gcum[up] - gcum[low];
        ans += (m - (up - low)) * b[i];
    }
