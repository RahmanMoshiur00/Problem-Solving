...
int main()
{
    intl n, t, A = 0LL, B = 0LL; cin>>n;
    priority_queue<intl> p1, p2;

    for(int i=0; i<n; i++) cin>>t, p1.push(t);
    for(int i=0; i<n; i++) cin>>t, p2.push(t);

    for(int i=1; i<=2*n; i++){
        if(i&1){
            if(!p1.empty() && !p2.empty()){
                if(p1.top() >= p2.top()) A += p1.top() , p1.pop();
                else p2.pop();
            }
            else if(!p1.empty()) A += p1.top() , p1.pop();
            else if(!p2.empty()) p2.pop();
        }
        else{
            if(!p1.empty() && !p2.empty()){
                if(p2.top() >= p1.top()) B += p2.top(), p2.pop();
                else p1.pop();
            }
            else if(!p2.empty()) B += p2.top() , p2.pop();
            else if(!p1.empty()) p1.pop();
        }
