...
{
    int n; cin>>n;
    int speed[n], ram[n], hdd[n], price[n], speed_max, ram_max, hdd_max;

    bool buyable[n];
    for(int i=0; i<n; i++) buyable[i] = true;

    speed_max = ram_max = hdd_max = INT_MIN;
    for(int i=0; i<n; i++){
    ...
    int ans, price_min = INT_MAX;
    for(int i=0; i<n; i++){
        if(buyable[i] && price_min>price[i]) price_min = price[i] , ans = i+1;
    }
