    ...
    while(cin>>n){
        ...
        for(int i=0; i<n; i++){
            ...
            if(c==1){
                ...
                del++;
                if(!st.empty() && st.top()==v) st.pop() , stn++;
                if(!qu.empty() && qu.front()==v) qu.pop() , qun++;
                if(!pq.empty() && pq.top()==v) pq.pop() , pqn++;
            }
        }

        if((del==stn && del==qun) || (del==stn && del==pqn) || (del==qun && del==pqn)) cout<<"not sure"<<endl;
        else if(del == stn) cout<<"stack"<<endl;
        else if(del == qun) cout<<"queue"<<endl;
        else if(del == pqn) cout<<"priority queue"<<endl;
