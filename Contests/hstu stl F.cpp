...
int main()
{
    ...
    while(t--){
        ...
        map<string, int> mp;
        while(n--) cin>>ingred>>d , mp[ingred] = d;
        getchar();

        vector< pair<intl, string> > ans;
        while(r--){
            getline(cin, recipe);
            cin>>k;
            intl cost = 0LL;
            ...
            if(cost<=b) ans.pb(MP(cost, recipe));
        }

        sort(all(ans));

        for(int i=0; binder[i]; i++){
            if(binder[i]>='a' && binder[i]<='z') binder[i] = binder[i] + 'A' - 'a';
            cout<<binder[i];
        } cout<<endl;

        if(ans.size()){
            for(int i=0; i<ans.size(); i++) cout<<ans[i].ss<<endl;
        }
