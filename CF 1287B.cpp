...
bool ok(string s, string e, string t)
{
    set<char> st;
    for(int i=0; i<s.size(); i++){
        st.clear();
        st.insert(s[i]); st.insert(e[i]); st.insert(t[i]);
        if(!(st.size() == 1 || st.size() == 3)) return false;
    }
    return true;
}
...
void solve(int tc)
{
    ...
    for(int i=0; i<n; i++){
        ...
        else if(str[0] == 'T') t.push_back(str);
        else e.push_back(str);
    }
