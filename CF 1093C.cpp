...
intl n, l, f, hi, lo, mid;
intl b[mxn];

bool ok(intl x, intl y)
{
    return  (x>=f and y<=l);
}
...
void solve(int tc)
{
    ...
    rep(i, 0, vf.size()) pfl(vf[i]), space;
    rep(i, vl.size(), 0) pfl(vl[i]), space;
}
