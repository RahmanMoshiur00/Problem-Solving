...
void solve(int tc)
{
    ...
    rep (i, 0, n) sfi2(vp[i].first, vp[i].second);

    int ans3 = 0;
    rep(i, 0, n){
        ans3 += (i * vp[i].first) + ((n-i-1) * vp[i].second);
    }
    ...
    int ans1 = 0, ans2 = 0;
    rep(i, 0, n){
        ans1 += (i * vp[i].first) + ((n-i-1) * vp[i].second);
    }

    sort(all(vp), cmp2);

    rep(i, 0, n){
        ans2 += (i * vp[i].first) + ((n-i-1) * vp[i].second);
    }

    debug cout<<"ans: "<<ans1<<sp<<ans2<<sp<<ans3<<endl;
