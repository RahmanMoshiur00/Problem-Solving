...
struct info{
    ...
    info(){
        sum = 0;
        prop = -1; // propagation value can be zero. so storing -1
    }
} tree[4 * mxn];
...
void propagate(int ind, int b, int e)
{
    if(b != e){
        int left = ind << 1;
        int right = left + 1;
        tree[left].prop = tree[right].prop = tree[ind].prop;
    }
    tree[ind].sum = (e-b+1) * tree[ind].prop;
    tree[ind].prop = -1; // propagation value can be zero. so storing -1
}

void update(int ind, int b, int e, int l, int r, int v)
{
    if(tree[ind].prop != -1) propagate(ind, b, e);
