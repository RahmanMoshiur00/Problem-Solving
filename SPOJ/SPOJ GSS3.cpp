...
{
    if(l==r){
        seg[ind].prefix = seg[ind].total = seg[ind].suffix = seg[ind].maxsum = y;
        return;
    }
    ...
    if(x <= mid) update(left, l, mid, x, y);
    else update(right, mid+1, r, x, y);
...
void solve(int tc)
{
    ...
    while(q--){
        sfi3(t, l, r);
        if(t==1) ans = query(1, 1, n, l, r), pfin(ans.maxsum);
