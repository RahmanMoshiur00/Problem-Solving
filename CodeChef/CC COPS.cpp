...
int main()
{
    ...
    while(t--){
        ...
        bool isCopHouse[mxn];
        fill(isCopHouse, isCopHouse+mxn, false);

        vector<int> cops(m);
        FOR(i, 0, m-1) sfi(cops[i]), isCopHouse[cops[i]] = true;

        int nextCopHouse[101], next = INT_MAX;
        ROF(i, 100, 1){
            if(isCopHouse[i]){
                next = i;
            }
        ...
        int prevCopHouse[101], prev = INT_MIN;
        FOR(i, 1, 100){
            if(isCopHouse[i]){
                prev = i;
            }
        ...
        int ans = 0;
        FOR(i, 1, 100){
            if(prevCopHouse[i]+x>=i || nextCopHouse[i]-x<=i) continue; //i is reachable i.e not safe
            else ans++;
        }
