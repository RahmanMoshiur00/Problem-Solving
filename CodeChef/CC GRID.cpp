...
int main()
{
    ...
    while(tc--){
        ...
        bool right[n+1][n+1], down[n+1][n+1];

        FOR(i, 0, n) right[i][n] = down[n][i] = true;

        ROF(i, n-1, 0){
            ROF(j, n-1, 0){
                if(s[i][j] == '.' && right[i][j+1]) right[i][j] = true;
                else right[i][j] = false;

                if(s[j][i] == '.' && down[j+1][i]) down[j][i] = true;
                else down[j][i] = false;
            }
        }
