...
void solve(int tc)
{
    ...
    for(int i=0; i<n; i++){
        cin >> x >> y;
        vp.push_back({x, 1}); // at point x; 1 is used as one customer increased
        vp.push_back({y+1, -1}); // at point y; -1 is used as one customer decreased
    }
    ...
    int ans = 0, sum = 0;
    for(auto &it: vp){
        sum += it.second; // sum indicates how many customer is in at any interval points
        ans = max(ans, sum);
    }
