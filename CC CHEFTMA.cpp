...
int main()
{
    ...
    while(tc--){
        ...
        FOR(i, 0, N-1) cin>>a[i];
        FOR(i, 0, N-1) cin>>t , a[i] = a[i] - t;
        FOR(i, 0, M+K-1) cin>>t , b.insert(t);

        sort(a, a+N);

        intl val, temp , ans = 0LL;
        multiset<intl>::iterator it;

        for(int i=N-1; i>=0; i--){
            if(b.empty() || *b.begin()>a[i]) ans += a[i];
            else{
                auto it = b.upper_bound(a[i]);
                it--;
                ans += a[i] - (*it);
                b.erase(it); //must use iterator parameter, if *it is used then all x==*it will be deleted from multiset
            }
        }
