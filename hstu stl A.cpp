...
int main()
{
    ...
    while(getline(cin, str)){
        ...
        for(int i=0; str[i]; i++){
            ...
            else if(!space) meaning += str[i];
            else if(space) word += str[i];
        }
        if(meaning!="" && word!="") mp[word] = meaning;
        else if(meaning!=""){
            if(mp.find(meaning) != mp.end()) cout<<mp[meaning]<<endl;
            else cout<<"eh"<<endl;
        }
    }
