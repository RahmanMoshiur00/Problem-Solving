...
void build(int ind, int b, int e)
{
    ...
    tree[ind].prop = 0;
    rep(i, 0, 3) tree[ind].mod[i] = tree[left].mod[i] + tree[right].mod[i];
}
...
void propagate(int ind, int b, int e)
{
    if(b != e){
        int left = ind << 1;
        int right = left + 1;
        tree[left].prop = (tree[left].prop + tree[ind].prop) % 3;
        tree[right].prop = (tree[right].prop + tree[ind].prop) % 3;
    }
    rep(i, 0, tree[ind].prop){
        swap(tree[ind].mod[0], tree[ind].mod[1]);
        swap(tree[ind].mod[0], tree[ind].mod[2]);
    }
    tree[ind].prop = 0;
}

void update(int ind, int b, int e, int l, int r)
{
    if(tree[ind].prop) propagate(ind, b, e);

    if(e<l || b>r) return;
    if(b>=l && e<=r){ // updating with propagation
        tree[ind].prop += 1;
        propagate(ind, b, e);
        return;
    }
