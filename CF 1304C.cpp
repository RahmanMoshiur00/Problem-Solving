...
void solve(int tc)
{
    ...
    int t[n], l[n], r[n], dp[n][2], tym;
    for(int i=0; i<n; i++) cin >> t[i] >> l[i] >> r[i];

    dp[0][0] = max(l[0], m-t[0]);
    dp[0][1] = min(r[0], m+t[0]);

    if(l[0] > dp[0][1] || dp[0][0] > r[0]) {
        cout << "NO\n";
        return;
    }
    ...
    for(int i=1; i<n; i++){
        ...
        dp[i][0] = max(l[i], dp[i-1][0] - tym);
        dp[i][1] = min(r[i], dp[i-1][1] + tym);

        if(l[i] > dp[i][1] || dp[i][0] > r[i]) {
            cout << "NO\n";
            return;
        }
    }
