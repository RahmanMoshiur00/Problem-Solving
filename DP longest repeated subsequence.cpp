...
int LCS(string& str1, string& str2){ // we have to find subsequence of str2 in str1
    ...
    for(int i=0; i<=len1; i++){
        for(int j=0; j<=len2; j++){
            if(i==0 || j==0) dp[i][j] = 0;
            else if(str1[i-1] == str2[j-1] && i != j){ //because str1, str2 starts from index 0; but we're calculating from 1
                dp[i][j] = dp[i-1][j-1] + 1;
            }
...
void printPath(string& str1, string& str2, int len1, int len2){
    ...
    else if(str1[len1-1] == str2[len2-1] && len1!=len2){
...
int main()
{
    ...
    while(t--){
        cout<<"\nEnter a string to find its Longest Repeated Subsequence: ";
        string str; cin>>str;

        int L = str.length();

        cout<<LCS(str, str)<<endl;
        cout<<"Printing path: ";
        printPath(str, str, L, L);
