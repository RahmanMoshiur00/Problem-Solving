...
int l, r, d, u, x, y, x1, y11, x2, y2;

bool inBoundary(int xat, int yat)
{
    return ((x1<=xat && xat<=x2) && (y11 <= yat && yat<=y2));
}

bool ok()
{
    return ((inBoundary(x-1, y) || inBoundary(x+1, y)) &&
    (inBoundary(x, y-1) || inBoundary(x, y+1)));
}
...
void solve(int tc)
{
    ...
    int rl = r - l, ud = u - d;
    int xat = x + rl, yat = y + ud;

    if(r+l>0 && !inBoundary(x+1, y) && !inBoundary(x-1, y)) cout << "NO\n";
    else if(u+d>0 && !inBoundary(x, y-1) && !inBoundary(x, y+1)) cout << "NO\n";
    else{
        if(inBoundary(xat, yat)) cout << "YES\n";
        else cout << "NO\n";
    }
...
//    int lr = a - b, du = c - d;
//    int xat = x - lr, yat = y - du;
//    if(ok() && inBoundary(xat, yat)) cout << "Yes\n";
//    else cout << "No\n";
}
