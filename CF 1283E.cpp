...
void solve(int tc)
{
    ...
    for(int i=0; i<n; i++){
        cin >> arr[i];
        if(chk[ arr[i] ] == 0) flag[i] = 1, mx++;
        chk[arr[i]] = 1;
    }
    for(int i=0; i<n; i++){
        if(flag[i] == 0){
            if(arr[i]-1 >= 1  && chk[arr[i]-1]==0){
                chk[arr[i]-1] = 1;
                mx++;
            }
            else if(arr[i]+1 <= n  && chk[arr[i]+1]==0){
                chk[arr[i]+1] = 1;
                mx++;
            }
        }
    ...
    int mn = n, seg;
    for(int i=0; i<n; i++){
        seg = upper_bound(arr, arr+n, arr[i]+2) - lower_bound(arr, arr+n, arr[i]);
        mn = mn - seg + 1;
        i = i + seg - 1;
    }
