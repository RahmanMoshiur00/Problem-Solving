...
int dp[mxn], coins[mxn], dist[mxn];

int main()
{

    int tc, n, val, t; sfi(tc);
    while(tc--){
        sfi(n);

        for(int i=0; i<n; i++) sfi(dist[i]), dist[i] *= 2;
        for(int i=0; i<n; i++) sfi(coins[i]);

        int mx_dist = *max_element(dist, dist+n);
        ...
        int ans = 0;
        for(int i=0; i<n; i++) ans += dp[dist[i]];
