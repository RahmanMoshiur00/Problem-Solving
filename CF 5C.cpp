...
char str[mxn];
int beg[mxn], open[mxn]; // beg[] = position of the earliest opening bracket
                         // open[] = position of the open bracket position of the corresponding ending bracket
...
int main()
{
    ...
    for(int i=1; str[i]; i++){
        if(str[i]=='(') st.push(i);
            ...
            if(st.empty()) beg[i] = open[i] = -1;
            else{
                open[i] = st.top() , st.pop();
                if(beg[open[i]-1]<=0) beg[i] = i - open[i] + 1;
                else beg[i] = beg[open[i]-1] + i - open[i] + 1;
            }
        }
    }

    int mx = *max_element(beg, beg+mxn);

    if(mx == 0) cout<<"0 1"<<endl;
    else{
        int cnt = 0;
        for(int i=1; str[i]; i++) if(beg[i]==mx) cnt++;
        cout<<mx<<sp<<cnt<<endl;
    }
