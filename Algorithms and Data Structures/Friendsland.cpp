...
void solve(int tc)
{
    ...
    rep (i, 0, n){
        ...
        mnarr[i] = window.front().first;
    }

    int mxarr[n];

    window = deque< pair<int,int> > ();
    rep (i, 0, n) {
        while (!window.empty() && window.back().first <= arr[i]){
            window.pop_back();
        }
        ...
        mxarr[i] = window.front().first;
    }

    debug{
        rep(i, 0, n) cout<<i+1<<": "<<mxarr[i]<<sp<<mnarr[i]<<endl;
    }

    vector<int> res;
    rep (i, 0, n) {
        if(i>=k-1) {
            if(abs(mxarr[i] - mnarr[i]) <= c) res.push_back(i-k+2);
        }
    }

    if(res.empty()) puts("NONE");
    else{
        rep(i, 0, res.size()) pfin(res[i]);
    }
}
